...
`surgery_[_[[i]]_intent`|`CategoricalDtype`|✅|`concept_name` of `measurement.measurement_concept_id` where `measurement_concept_id` is one of `4179711`, `4162591` and `measurement_event_id` is equal to `procedure_occurrence.procedure_occurrence_id` which is equal to `episode_event.event_id` and `episode_event.episode_id` is equal to `episode.episode_id` where `episode.episode_concept_id = 32939`
`surgery_[_[[i]]_margins_after_surgery`|`CategoricalDtype`|✅|`concept_name` of `measurement.measurement_concept_id` where `measurement_concept_id` is one of `1634643`, `1633801`, `1634484` and `measurement_event_id` is equal to `procedure_occurrence.procedure_occurrence_id` which is equal to `episode_event.event_id` and `episode_event.episode_id` is equal to `episode.episode_id` where `episode.episode_concept_id = 32939`
`surgery_[_[[i]]_date`|`datetime64[ns, tz]`|✅||
`surgery_[_[[i]]__intraoperative_radio`|`boolean`|✅|`TRUE` if `radio_start_date` is equal to one of `procedure_occurrence.procedure_date` where `procedure_occurrence_id` is equal to `episode_event.event_id` and `episode_event.episode_id` is equal to `episode.episode_id` where `episode.episode_concept_id = 32940` (⚠️ For H&N patients multiple procedures may be associated with the same radiotherapy episode)
...
`[pre_operative/post_operative/recurrence]_radio[_[i]]_intent`|`CategoricalDtype`|✅|`concept_name` of `measurement.measurement_concept_id` where `measurement_concept_id` is one of `4179711`, `4162591` and `measurement_event_id` is equal to `procedure_occurrence.procedure_occurrence_id` which is equal to `episode_event.event_id` and `episode_event.episode_id` is equal to `episode.episode_id` where `episode.episode_concept_id = 32940`
`[pre_operative/post_operative/recurrence]_radio[_[i]]_hospital`|`CategoricalDtype`|✅|`provider_name` of `provider.provider_id` where `provider_id` is equal to (`procedure_occurrence.provider_id` where `procedure_occurrence_id` is equal to `episode_event.event_id` and `episode_event.episode_id` is equal to `episode.episode_id` where `episode.episode_concept_id = 32940`)
`[pre_operative/post_operative/recurrence]_radio[_[i]]_start_date`|`datetime64[ns, tz]`|✅|`episode_start_date` of `episode.episode_concept_id = 32940`
`[pre_operative/post_operative/recurrence]_radio[_[i]]_end_date`|`datetime64[ns, tz]`|✅|`episode_end_date` of `episode.episode_concept_id = 32940`
`[pre_operative/post_operative/recurrence]_radio[_[i]]_setting`|`CategoricalDtype`(?)|✅|Derived, with settings included in the variable name
`[pre_operative/post_operative/recurrence]_radio[_[i]]_total_dose_gy`|`Float64`|✅|`value_as_number` of `measurement.measurement_concept_id` where `measurement_concept_id = 40483776` and `measurement_event_id` is equal to `procedure_occurrence.procedure_occurrence_id` which is equal to `episode_event.event_id` and `episode_event.episode_id` is equal to `episode.episode_id` where `episode.episode_concept_id = 32940`
`[pre_operative/post_operative/recurrence]_radio[_[i]]_number_of_fractions`|`Float64`|✅|`value_as_number` of `measurement.measurement_concept_id` where `measurement_concept_id = 4037631` and `measurement_event_id` is equal to `procedure_occurrence.procedure_occurrence_id` which is equal to `episode_event.event_id` and `episode_event.episode_id` is equal to `episode.episode_id` where `episode.episode_concept_id = 32940`
`[pre_operative/post_operative/recurrence]_radio[_[i]]_treatment_completed_as_planned`|`CategoricalDtype`|✅|`concept_name` of `measurement.measurement_concept_id` where `measurement_concept_id` is one of `44788181`, `4161588`, `2000100030`, `4105297`, `37017062`, `4306655` and `measurement_event_id` is equal to `procedure_occurrence.procedure_occurrence_id` which is equal to `episode_event.event_id` and `episode_event.episode_id` is equal to `episode.episode_id` where `episode.episode_concept_id = 32940`
...
`[pre_operative/post_operative/recurrence]_systemic_treatment_[i]_drugs_for_treatments_[j]`|`CategoricalDtype`|✅|`concept_name` of `drug_exposure.drug_concept_id` where `drug_exposure_start_date` is equal to `systemic_start_date_systemic_treatment` and `drug_exposure_end_date` is equal to `systemic_end_date_systemic_treatment` (maximum number of drugs = 10)
`[pre_operative/post_operative/recurrence]_systemic_treatment_[i]_drugs_for_treatments_count`|`CategoricalDtype`|✅|count of all drugs per treatment
`overall_treatment_response_response`|`CategoricalDtype`|✅|`concept_name` of `episode.episode_concept_id` where `episode_concept_id` is one of `32946`, `32947`, `32948`, `32949` and `episode_start_date` is between last treatment end date (any of the defined treatment, e.g. radio, systemic, surgery) (included, ≥) and the first recurrence (see [Treatment settings](#treatment-settings-) for the definition of the recurrence) (excluded, <)
`overall_treatment_response_defined_done`|???|❌|This is not included in OMOP because it does not comply with the OMOP CDM structure
`overall_treatment_response_date`|`datetime64[ns, tz]`|✅|`episode_start_date` of `episode.episode_concept_id` where `episode_concept_id` is one of `32946`, `32947`, `32948`, `32949` and `episode_start_date` is between last treatment end date (any of the defined treatment, e.g. radio, systemic, surgery) (included, ≥) and the first recurrence (see [Treatment settings](#treatment-settings-) for the definition of the recurrence) (excluded, <)
...
`clinical_metastasis_at_bone`|`boolean`|✅|`TRUE` if is present `measurement.measurement_concept_id = 36769301` where `measurement.measurement_date` is equal to `diagnosis_date`
...
`pathological_metastasis_at_bone`|`boolean`|✅|`TRUE` if is present `measurement.measurement_concept_id = 36769301` where `measurement.measurement_date` is equal to `date_of_surgery`**
...
Variable|Type|Status|Notes
--|--|--|--
`surgery_[_[[i]]_surgery_hospital`|`CategoricalDtype`|✅|`provider_name` where `provider.provider_id` is equal to `procedure_occurrence.provider_id` for which `procedure_occurrence.procedure_occurrence_id` is equal to `episode_event.event_id` and `episode_event.episode_id` is equal to `episode.episode_id` where `episode.episode_concept_id = 32939`
...
`surgery_[_[[i]]_extra_nodal_extension`|`CategoricalDtype`|✅|`concept_name` of `measurement.measurement_concept_id` where `measurement_concept_id` is one of `36769946`, `36770618` and `measurement_event_id` is equal to `procedure_occurrence.procedure_occurrence_id` which is equal to `episode_event.event_id` and `episode_event.episode_id` is equal to `episode.episode_id` where `episode.episode_concept_id = 32939`
...
`surgery_[_[[i]]_laterality_of_the_dissection`|`CategoricalDtype`|✅|`concept_name` of `measurement.measurement_concept_id` where `measurement_concept_id` is one of `4112106`, `4117496`, `4112107` and `measurement_event_id` is equal to `procedure_occurrence.procedure_occurrence_id` which is equal to `episode_event.event_id` and `procedure_occurrence.procedure_concept_id = 4291481` and `episode_event.episode_id` is equal to `episode.episode_id` where `episode.episode_concept_id = 32939`
`surgery_[i]_date_of_neck_surgery`|`datetime64[ns,tz]`|✅|`procedure_date` of `procedure_occurrence.procedure_concept_id = 4291481` where `procedure_occurrence_id` is equal to `episode_event.event_id` and `episode_event.episode_id` is equal to `episode.episode_id` where `episode.episode_concept_id = 32939`
`[pre_operative/post_operative/recurrence]_radio[_[i]]_beam_quality`|`CategoricalDtype`|✅|`concept_name` of `procedure.procedure_concept_id` where `procedure_concept_id` is one of (`607996`,`4165039`,`607995`,`4024005`,`4012485`,`4161415`,`4058601`,`4077953`) and `procedure_occurrence_id` is equal to `episode_event.event_id` and `episode_event.episode_id` is equal to `episode.episode_id` where `episode.episode_concept_id = 32940`
`[pre_operative/post_operative/recurrence]_radio[_[i]]_total_high_dose`|`Float64`|✅|`value_as_number` of `measurement.measurement_concept_id` where `measurement_concept_id = 4129296` and `measurement_event_id` is equal to `procedure_occurrence.procedure_occurrence_id` which is equal to `episode_event.event_id` and `episode_event.episode_id` is equal to `episode.episode_id` where `episode.episode_concept_id = 32940`
`radio_treatment_site_primary_only`|`boolean`|❌|They are not included in OMOP because no corresponding codes/mappings have been defined in the data model
`radio_treatment_site_neck_only`| `boolean`|❌|They are not included in OMOP because no corresponding codes/mappings have been defined in the data model
`radio_treatment_site_primary_and_ipsilateral_neck`|`boolean`|❌|They are not included in OMOP because no corresponding codes/mappings have been defined in the data model
`radio_treatment_site_primary_and_bilateral_neck`|`boolean`|❌|They are not included in OMOP because no corresponding codes/mappings have been defined in the data model
`[pre_operative/post_operative/recurrence]_radio[_[i]]_treatment_site_distant_metastasis`|`boolean`|✅|`TRUE` if `procedure.modifier_concept_id IS NOT NULL`  where `procedure_occurrence_id` is equal to `episode_event.event_id` and `episode_event.episode_id` is equal to `episode.episode_id` where `episode.episode_concept_id = 32940`
`[pre_operative/post_operative/recurrence]_systemic_treatment[_[i]]_intent`|`CategoricalDtype`|✅|`concept_name` of `measurement.measurement_concept_id` where `measurement_concept_id` is one of `4179711`, `4162591` and `measurement_event_id` is equal to `procedure_occurrence.procedure_occurrence_id` which is equal to `episode_event.event_id` and `episode_event.episode_id` is equal to `episode.episode_id` where `episode.episode_concept_id = 32531`
`[pre_operative/post_operative/recurrence]_systemic_treatment[_[i]]_setting`|`CategoricalDtype`|✅|This will be derived using dates ⚠️ added to basic variables (not only H&N)
