...
Variable|Type|Status|Notes
--|--|--|--
`surgery_hospital`|`CategoricalDtype`|✅|`provider_name` where `provider.provider_id` is equal to `procedure_occurrence.provider_id` for which `procedure_occurrence.procedure_occurrence_id` is equal to `episode_event.event_id` and `episode_event.episode_id` is equal to `episode.episode_id` where `episode.episode_concept_id = 32939`
